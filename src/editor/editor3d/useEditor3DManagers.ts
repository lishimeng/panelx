...
export function useEditor3DManagers(options: UseEditor3DManagersOptions) {
  ...
  function executeProperty(req: PropertyJsonExecuteRequest): void {
    ...
    if (!raw) {
      const msg = '[Editor3D] executeProperty: ËØ∑Ê±? JSON ‰∏çË?Ω‰∏∫Á©∫'
      console.error(msg)
      setPropertyError('ËØ∑Ê±? JSON ‰∏çË?Ω‰∏∫Á©∫')
      return
    }
    try {
      ...
      if (!key || !id) {
        const msg = '[Editor3D] executeProperty: JSON Âø?È°ªÂ??Âê´Èù?Á©∫ key ÔøΩ?id'
        console.error(msg, parsed)
        setPropertyError('JSON Âø?È°ªÂ??Âê´Èù?Á©∫ key ÔøΩ?id')
        return
      }
      propertyRequestError.value = ''
      propertyManager.execute({ key, id, params: parsed?.params })
    } catch (err) {
      const msg = '[Editor3D] executeProperty: JSON Ëß£Ê?êÂ§±Ë¥•'
      console.error(msg, err)
      setPropertyError('JSON Ëß£Ê?êÂ§±Ë¥•')
    }
  }
