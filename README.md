...
- **`widgetData`**（`src/components/Dashboard.vue`）  
  - 类型：`Ref<WidgetDataMap>`，即 `Record<string, Record<string, unknown>>`（键为组件 **`id`**）。  
...
- **统一事件模型**：`ControlEnvelope -> ControlPayload`；其中 `command` / `property` / `camera` / `widget` 四类载荷的**业务体**均为 **`ControlRequest`**（`{ key, id?, params? }`）。`kind: 'widget'` 时约定 `key === WIDGET_PATCH_REQUEST_KEY`（`widget.patch`），`params` 为含 **`patch`** / 可选 **`refresh`** 的普通对象（见 `src/types/manager.ts` 注释与 `widgetPayloadPatch`）。
...
- **`DatasourceRoutedEvent`**（`kind: 'datasource_routed'`，见 `src/types/manager.ts`）：Dashboard 内置 SSE/polling 解析结果可走此分支，**不经** `widget.patch.__routed` 嵌套；`SSESource` / `PollingSource` 在 `normalizeControlEnvelope` 之前识别该形状并原样 `push`。`StreamEngine` 队列**不**消费该事件（若误注册会记日志并丢弃）。

说明：

- 路由解析在各 datasource 内完成（例如 Dashboard 的 SSE 优先用 **`event:` 行** 解析 `domain/action`，`data` 为 **`ControlRequest`**）。
...
大屏通过 `Dashboard` 内配置的 **`type: 'sse'`** 数据源连接后端。**客户端只认线上标准帧**：**`event:` = 路由**，**`data:` = 一段 `ControlRequest` JSON**（可多行 `data:` 拼成一段）。**不会**把 `{ header, payload[] }` 当作线上协议解析。解析逻辑见 `src/components/Dashboard.vue` 中 `parseMessage`。

#### 测试数据文件（`server/data/*_enable.json`，仅磁盘格式）

仓库里 **`pnpm run sse`** 用的 JSON **允许**使用 **`header`（含 `route`、`strategy`）+ `payload` 数组**，便于一条文件里放多组测试样例。**仅服务端读文件时用**；`server/sse-server.mjs` 加载后会**逐条**写出 SSE：**每一帧**都是 `event: {domain}_{action}` + `data: { key, id, params }`，**不会**把整包 `header/payload` 原样推给浏览器。
...
- **线上**：路由写在 SSE 的 **`event:`** 上（如 `event: 2d_chart`），与 `DEFAULT_SSE_NAMED_EVENTS` 对齐。
...
未使用命名 `event:`、或需在 **`data` 数组**里为每一项单独指定路由时：每一项可通过 **`event`**（`{domain}_{action}`）或 **`domain` + `action`** 解析路由；体须为 **`ControlRequest`**（`key` / `id` / `params`）。

#### SSE 传输层说明

- **`SSESource`**（`src/utils/controlSources/SSESource.ts`）：使用浏览器 `EventSource`。除默认 `message` 外，还会监听一组命名事件，以便与服务端 `event: xxx` 对齐；`parseMessage` 的**第二参数**为浏览器提供的 **`MessageEvent.type`**（即 `event:` 名称）。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| （传输层）`event:` | `string` | 推荐 | SSE 帧上的路由，如 `2d_chart` / `2d_property` |
| `id` | `string` | 通常必填 | 目标 2D 组件实例 id（`3d_camera` 等特例除外） |
| `key` / `params` | `string` / `object` | 按业务 | 与 **`ControlRequest`** 一致；图表类常见为 `params` 内放 options 或 `params.patch` |

3D（经 SSE 路由为 `3d_*` 时，`data` 为 **`ControlRequest`**）建议字段：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `key` | `string` | 是 | handler key，如 `editor3d.moveTo` / `model.visible` / `camera.moveTo` |
| `id` | `string` | 条件必填 | `command/property` 通常必填（模型实例 id）；`camera` 可不填 |
| `params` | `object` | 否 | 业务参数；不同 key 结构不同 |

进入 `StreamEngine` 后的 **`ControlEnvelope`** 仍可带 `sourceId` / `timestamp` / `traceId` / `priority` 等元数据（由 source 层补全）。

批量事件建议：

- polling 接口返回 **数组**或**单对象**；每个元素为 **`ControlRequest`**，并带 **`event`** 或 **`domain`+`action`**（若 HTTP 体无法表达 SSE 的 `event:`）。
- SSE：**每条消息**一条标准帧；多组数据应对应**多次** `write`（或单帧 `data` 为 JSON 数组，由客户端展开），**不要**在线上使用 `header`+`payload[]` 信封。
- Dashboard 内所有后端数据都必须先进入 dataEngine，再路由到 2D widget 或 3D manager。

Camera 事件示例（`event: 3d_camera`，`data` 为 **`ControlRequest`**）：
